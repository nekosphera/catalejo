...
SHAPE_NAMESPACES = {
    ...
    "myds": "https://www.mydataspace.es/ns#",
...
def metadata_aliases():
    ...
    return {
        ...
        "myds:deliveryMode": ["deliveryMode", "delivery_mode"],
    }
...
class CatalogueProfile:
    ...
    def validate(self, metadata):
        ...
        delivery_mode = str(normalized.get("myds:deliveryMode") or "").strip()
        if access_rights in {"contractual-dashboard", "controlled-governed-reuse"} and not delivery_mode:
            recommendations.append(
                "set myds:deliveryMode so policy enforcement can choose the right contract path"
            )
